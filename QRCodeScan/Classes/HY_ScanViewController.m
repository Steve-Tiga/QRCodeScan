...
#import "HY_ScanViewController.h"
...
@interface HY_ScanViewController ()<UINavigationControllerDelegate, UIImagePickerControllerDelegate, AVCaptureMetadataOutputObjectsDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) NSString *appName;
@property (nonatomic, strong) NSBundle *scanBundle;

@property (nonatomic, strong) HY_ScanView *scanRectView;
...
@property (nonatomic, assign) HY_ScanType scanType;  //扫码类型
...
@implementation HY_ScanViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // 将状态栏和导航条设置成透明
    UIColor *color = [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:0];
    UIImage *image = [HY_ScanCodeTool createImageWithColor:color];
...
/// 初始化扫描类型
- (void)configScanType {
    if (self.scanType == HY_ScanTypeAll) {
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
        [self drawBottomItems];
    } else if (self.scanType == HY_ScanTypeQrCode) {
        self.output.metadataObjectTypes = @[AVMetadataObjectTypeQRCode];
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
    ...
    } else if (self.scanType == HY_ScanTypeBarCode) {
        ...
        self.output.rectOfInterest = _scanRect;
        [self.scanRectView setScanType: HY_ScanTypeBarCode];
...
//绘制扫描区域
- (void)configScanView {
    _scanRectView = [[HY_ScanView alloc] initWithFrame:self.view.frame style:@""];
...
- (void)setNavItem:(HY_ScanType)type {
    if(type == HY_ScanTypeBarCode) {
...
- (void)qrBtnClicked:(UIButton *)sender {
    ...
    [_scanTypeBarBtn setSelected:NO];
    [self changeScanCodeType:HY_ScanTypeQrCode];
    [self setNavItem:HY_ScanTypeQrCode];
...
- (void)barBtnClicked:(UIButton *)sender {
    ...
    [self changeScanCodeType:HY_ScanTypeBarCode];
    [self setNavItem:HY_ScanTypeBarCode];
...
- (void)changeScanCodeType:(HY_ScanType)type {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self->_tipTitle.text = type == HY_ScanTypeQrCode ?
        @"将取景框对准二维码,即可自动扫描" :
        @"将取景框对准条码,即可自动扫描";
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        [self->_flashBtn setCenter:CGPointMake(self.view.center.x, type == HY_ScanTypeQrCode ? (self.view.center.y + kFlash_Y_PAD(70)) : CGRectGetMaxY(self.view.frame)- bottomPadding)];
    }];
...
-(void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    __weak typeof (self)weakSelf = self;
    [HY_ScanCodeTool recognizeQrCodeImage:image onFinish:^(NSString * _Nonnull result) {
